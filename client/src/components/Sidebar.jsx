          ...
          <img src={user.imageUrl} alt="User avatar" className='w-14 rounded-full mx-auto' />
           ...
           <div className='px-6 mt-5 text-sm text-gray-600 font-medium'>
            {navItems.map(({to,label,Icon})=>(
              <NavLink key={to} to={to} end={to === '/ai'} onClick={()=>setSidebar(false)} className={({isActive})=>` px-3.5 py-2.5 flex item-center gap-3 rounded ${isActive? 'bg-gradient-to-r from-[#3C81F6] to-[#9234ea] text-white' : ''}`}>
                {({isActive})=>(
                  <>
